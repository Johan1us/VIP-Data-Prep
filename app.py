    ...
    for col, specs in required_columns.items():
        if col in df.columns:
            ...
            if specs['type'] == 'BOOLEAN':
                invalid_bool = ~df[col].isin([True, False, 1, 0, 'true', 'false', 'True', 'False', pd.NA, None, 'Ja', 'Nee'])
                if invalid_bool.any():
                    validation_errors['warnings'].append({
                ...
                try:
                    ...
                    if 'date_format' in specs and specs['date_format'] == 'yyyy':
                        invalid_years = ~dates.dt.strftime('%Y').astype(str).str.match(r'^\d{4}$')
            ...
            if 'allowed_values' in specs:
                invalid_values = ~df[col].isin(specs['allowed_values'])
                invalid_rows = df[col][invalid_values]
                if invalid_values.any():
                    # Convert values to strings and handle NaN/None values
                    invalid_values_list = [str(val) if pd.notna(val) else 'Leeg' for val in invalid_rows.unique()]
                    validation_errors['critical'].append({
                        'message': f"Ongeldige waarden in kolom '{col}'",
                        'details': f"Gevonden ongeldige waarden: [{', '.join(invalid_values_list)}]. Toegestane waarden zijn: [{', '.join(specs['allowed_values'])}]"
                    })
                ...
                if st.button("📤 Upload naar API"):
                    try:
                        with st.spinner('Bezig met uploaden...'):
                            ...
                            st.session_state.log_messages.append(f"🕐 {pd.Timestamp.now().strftime('%H:%M:%S')} - Start upload van {len(payload)} objecten")
                            ...
                            if response.status_code == 200:
                                ...
                                st.session_state.log_messages.append(f"✅ {pd.Timestamp.now().strftime('%H:%M:%S')} - Upload succesvol: {success_count} objecten verwerkt ({created_count} nieuw, {updated_count} bijgewerkt)")
                        ...
                        st.session_state.log_messages.append(f"❌ {pd.Timestamp.now().strftime('%H:%M:%S')} - Verbindingsfout")
                        st.error("❌ Kon geen verbinding maken met de API. Controleer uw internetverbinding.")
                    except Exception as e:
                        st.session_state.log_messages.append(f"❌ {pd.Timestamp.now().strftime('%H:%M:%S')} - Fout: {str(e)}")
