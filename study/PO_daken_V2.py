...
class RoofManager:
    ...
    def get_all_buildings(
        ...
        buildings = self.api.get_objects_by_type(
            object_type="Building", attributes=attributes, page_size=page_size
        )
    ...
    def _add_validation_lists(
        self, workbook: xlsxwriter.Workbook, lookup_sheet: xlsxwriter.Worksheet
    ) -> None:
        """Add validation lists to the lookup sheet"""
        # Dakpartner options
        for row_num, option in enumerate(
            self.METADATA["dakpartner"]["attributeValueOptions"]
        ):
        ...
        for row_num, option in enumerate(
            self.METADATA["projectleider"]["attributeValueOptions"]
        ):
        ...
        workbook.define_name(
            "BooleanList", f"='Lookup_Lists'!$C$1:$C${len(boolean_options)}"
        )

    def _add_column_validation(
        self, worksheet: xlsxwriter.Worksheet, df: pd.DataFrame, columns: List[str]
    ) -> None:
